...
def main():
    st.image("./src/veer_nobg.png", use_column_width=False, width=100)
    st.header("Chat with VEER")
    st.write("")

    prompt = st.text_input("Prompt please...", placeholder="Prompt", label_visibility="visible")
    temp = st.slider("Smartness", 0.0, 1.0, step=0.05)    #Hyper parameter - range[0-1]
